...
;* Constant Values 																								 *
;*******************************************************************                                                            
LCD_DAT        EQU PORTB  ; LCD data port connected to PORTB pins PB7 to PB0
LCD_CNTR       EQU PTJ    ; LCD control port connected to PTJ pins PJ6 (RS), PJ7 (E)
LCD_E          EQU $80    ; Pin for LCD's E-signal
LCD_RS         EQU $40    ; Pin for LCD's RS-signal

;*******************************************************************
;* Path Detection Threshold Values 																 *
;*******************************************************************       
PTH_A_INT      EQU $CA    ; Lowest threshold value for sensor A
PTH_B_INT      EQU $CC    ; Lowest threshold value for sensor B      
PTH_C_INT      EQU $CA    ; Lowest threshold value for sensor C
PTH_D_INT      EQU $CC    ; Lowest threshold value for sensor D

PTH_E_INT      EQU $90   ; Threshold value for Sensors E and F, shift right
PTH_F_INT      EQU $65   ; Threshold value for Sensors E and F, shift left

;*******************************************************************
;* Displacement Values 																						 *
;*******************************************************************
INC_DIS        EQU 400   ; Distance to move
FWD_DIS        EQU 1200  ; Distance to go forward
REV_DIS        EQU 1000  ; Distance to reverse
STR_DIS        EQU 1200  ; Distance
TRN_DIS        EQU 14800 ; Distance to turn
UTRN_DIS       EQU 12600 ; Distance to u-turn

;*******************************************************************
;* Intersection Values 																						 *
;*******************************************************************
PRI_PTH_INT    EQU 0 ; Primary route
SEC_PTH_INT    EQU 1 ; Secodonary route
...
LT_TRN         EQU 4 ; Left turn
BK_TRK         EQU 5 ; Backtrack
STND_BY        EQU 6 ; Stop
...
DISPATCHER:    CMPA  #START                     ; Compare accumulator with the START state
               BNE   NOT_START                  ; If not equal, branch to NOT_START
               JSR   START_ST                   ; Otherwise, jump to the START_ST subroutine
               RTS                              ; Return from DISPATCHER                          
;------------------------------------------------------------------------------------------
NOT_START:     CMPA  #FWD                       ; Compare accumulator with the FWD state
               BNE   NOT_FORWARD                ; If not equal, branch to NOT_FORWARD
               JMP   FWD_ST                     ; Otherwise, jump to the FWD_ST subroutine
               RTS                              ; Return from DISPATCHER           
;------------------------------------------------------------------------------------------
NOT_FORWARD:   CMPA  #RT_TRN                    ; Compare accumulator with the RT_TRN state
               ...
               RTS                             	; Return from DISPATCHER                          
;------------------------------------------------------------------------------------------
NOT_RT_TRN:    CMPA  #LT_TRN                    ; Compare accumulator with the LT_TRN state
               BNE   NOT_LT_TRN                 ; If not equal, branch to NOT_LT_TRN
               LDY   #1000                      ; Set a 20 ms delay for turns
               JSR   del_50us                   ; Execute a 50 microseconds delay subroutine
               JSR   LT_TRN_ST                  ; Jump to LT_TRN_ST subroutine for left turn
               RTS                              ; Return from DISPATCHER                             
;------------------------------------------------------------------------------------------
NOT_LT_TRN:    CMPA  #REV                       ; Compare accumulator with the REV state
               BNE   NOT_REVERSE                ; If not equal, branch to NOT_REVERSE
               JSR   REV_ST                     ; Otherwise, jump to the REV_ST subroutine
               RTS                              ; Return from DISPATCHER                             
;------------------------------------------------------------------------------------------
NOT_REVERSE:   CMPA  #BK_TRK                    ; Compare accumulator with the BK_TRK state
               BNE   NOT_BK_TRK                 ; If not equal, branch to NOT_BK_TRK
               JMP   BK_TRK_ST                  ; Otherwise, jump to the BK_TRK_ST subroutine
               RTS                              ; Return from DISPATCHER                 
;------------------------------------------------------------------------------------------
NOT_BK_TRK:    CMPA  #STND_BY                   ; Compare accumulator with the STND_BY state
...
NO_D_DETECT:   LDAA B_DETN                     ; load sensor B
               ...
               BEQ LT_TURN                     ; Sensor A = 1, jump to subroutibe
                            
               BRA NO_SHFT_LT                  ; Sensor A = 0, jump to subroutibe
                                               

LT_TURN:       JSR   INIT_FWD                   ; Initialize forward motion   
               MOVB  #FWD, CRNT_STATE           
               LDY   #570                       ; Delay movement to position robot at intersection 
               JSR   del_50us                   
               
               JSR   INIT_LT_TRN               ; Initialize left turn
...
NO_SHFT_RT:    JSR   STARON                    ; Turn motors on
               JSR   PORTON                    
...
;------------------------------------------------------------------------------------------
LT_TRN_ST:     LDD   COUNT1                    ; If current distance is more than forward distance then
               CPD   #STR_DIS                  ; Turn robot
               BLO   LT_TRN_ST                 
...
LT_TURN_DEL:   LDD   COUNT1                    ; If current distance is more than forward turn distance then
               CPD   #TRN_DIS                  ; Stop robot
               BLO   LT_TURN_DEL               
               JSR   INIT_FWD                  ; Initialize the FWD state
               LDAA  RETURN                    ; If RETURN = 1 
...
LT_TRN_EXIT:   RTS                             ; Return to main
...
BK_TRK_EXIT:   JMP   MAIN                      ; Return to main
